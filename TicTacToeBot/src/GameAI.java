...
public class GameAI
{
	...
	public void findBestBoardMinimax(GameState state)
	{
		...
		list = state.generateSuccessors(state, AI_LETTER);
		ArrayList<Integer> valueList = new ArrayList<Integer>();
		for(int i=0;i<list.size();i++){
			GameState check = list.get(i);
			valueList.add(Minimax.miniMax(check, AI_LETTER));
		}
		int limit = valueList.size();
...
//		Log.debug("Selected State Value " + winnerVal);
		...
		*/
		boardState=list.get(maxPos).getBoardState();
		showBoardState(boardState);
	}
